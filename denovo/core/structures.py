...
@dataclasses.dataclass
class Task(denovo.quirks.Element, collections.abc.Hashable, abc.ABC):
    """Node type for denovo Workflows.
...
class Workflow(DirectedGraph):
    ...
    def merge(self, item: Union[Graph, Adjacency, Edges, Matrix]) -> None:
        ...
        if isinstance(item, Graph):
            ...
            adjacency = edges_to_adjacency(source = item)
        elif isinstance(item, Matrix):
            adjacency = matrix_to_adjacency(source = item)
        elif isinstance(item, (MutableSequence, Tuple, Set)):
            adjacency = pipeline_to_adjacency(source = item)
